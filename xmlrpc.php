...
if ($options_cache['isxmlrpcenable'] == 'n') {
	error_message(500, 'Совет: сайт XMLRPC службы не открыт.');
}
if (!$HTTP_RAW_POST_DATA) {
	error_message(500, 'Ошибка: XML-RPC сервер может принять только POST данные');
}
...
if (trim($data) == '') {
	error_message(500, 'Ошибка: нет данных');
}
...
function mw_getPost($args) {
	...
	if (empty($post)) return error_message(404, 'Нет доступа');
...
function mw_getRecentPosts($args) {
	...
	if (empty($xml)) {
		error_massage(404, 'Ошибка файла');
	}
	$xml = "<array><data>$xml</data></array>";
	response($xml);
}
...
function mw_newMediaObject($args) {
	...
	if (!preg_match('/([^\/\:\*\?<>\|]+\.\w{2,6})|(\\{2}[^\/\:\*\?<>\|]+\.\w{2,6})/', $file['name'], $matches))
		error_message(500, 'Ошибка в имени файла');
	$filename = $matches[0];
	...
	if (!in_array($extension, $att_type)) {
		error_message(500, 'Ошибка типов файлов');
	}
	...
	if (!is_dir($uppath_root)) {
		...
		if ($ret === false) {
			error_message(500, 'Не удалось создать каталог загрузки файлов');
		}
	}
	if (!is_dir($uppath)) {
		umask(0);
		...
		if ($ret === false) {
			error_message(500, 'Нет доступа в каталог (content/uploadfile)');
		}
	}

	$fp = @fopen($attachpath, 'wb');
	if (!$fp)
		error_message(500, 'Файл не может быть записан');
	fwrite($fp, $bits);
	fclose($fp);
...
function login($username, $password) {
	...
	if (!LoginAuth::checkUser($username, $password , '', 'n')) {
		error_message(403, 'Ошибки имени пользователя или пароля');
		return false;
	}
	// 返回用户信息
