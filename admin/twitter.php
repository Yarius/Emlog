...
if ($action == 'getreply') {
	...
	foreach ($replys as $val) {
		 if ($val['hide'] == 'n') {
			...
			$act = "<span><a href=\"javascript: hidereply({$val['id']},{$tid});\">Не проверен</a></span> ";
		 } else {
			$style = "background-color:#FEE0E4";
			$act = "<span><a href=\"javascript: pubreply({$val['id']},{$tid});\">Одобрить</a></span> ";
		 }
		 $response .= "
		 <li id=\"reply_{$val['id']}\" style=\"{$style}\">
		 <span class=\"name\">{$val['name']}</span> {$val['content']}<span class=\"time\">{$val['date']}</span>{$act}
		 <a href=\"javascript: delreply({$val['id']},{$tid});\">Удалить</a> 
		 <em><a href=\"javascript:reply({$tid}, '@{$val['name']}：');\">Ответ</a></em>
		 </li>";
	}
	echo $response;
}
...
if ($action == 'reply') {
	...
	$response = "
		 ...
		 <span><a href=\"javascript: hidereply({$rid},{$tid});\">Проверить</a></span> 
		 <a href=\"javascript: delreply({$rid},{$tid});\">Удалить</a> 
		 <em><a href=\"javascript:reply({$tid}, '@{$name}: ');\">Ответ</a></em>
		 </li>";
	echo $response;
}
// 删除回复.
