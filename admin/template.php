...
if ($action == '') {
	...
	$tplForEm = !empty($tplForEmlog[1]) ? 'Для emlog：' . $tplForEmlog[1] : '';

	if (isset($tplAuthor[1])) {
		$tplAuthor = !empty($tplUrl[1]) ? "Автор: <a href=\"{$tplUrl[1]}\">{$tplAuthor[1]}</a>" : "Автор: {$tplAuthor[1]}";
	} else{
		$tplAuthor = '';
	}
	//模板列表
...
if ($action == 'upload_zip') {
	...
	if (!$zipfile || $zipfile['error'] >= 1 || empty($zipfile['tmp_name'])) {
		emMsg('Сбой при загрузке шаблона');
	}
