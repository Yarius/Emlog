...
function __autoload($class) {
	...
	if (file_exists(EMLOG_ROOT . '/include/model/' . $class . '.php')) {
		...
		emMsg($class . 'Не удалось загрузить');
	}
}
...
function pagination($count, $perlogs, $page, $url, $anchor = '') {
	...
	if ($page > 6)
		$re = "<a href=\"{$urlHome}$anchor\" title=\"первая\">&laquo;</a><em>...</em>$re";
	if ($page + 5 < $pnums)
		$re .= "<em>...</em> <a href=\"$url$pnums$anchor\" title=\"последняя\">&raquo;</a>";
	if ($pnums <= 1)
		$re = '';
	return $re;
}
...
function breakLog($content, $lid) {
	...
	if (!empty($a[1])) {
		$a[0].='<p class="readmore"><a href="' . Url::log($lid) . '">Читать далее&gt;&gt;</a></p>';
	}
	return $a[0];
}
...
function uploadFile($fileName, $errorNum, $tmpFile, $fileSize, $type, $isIcon = false, $is_thumbnail = true) {
	...
	switch ($result) {
		case '100':
			emMsg('Размер файла превышает максимально допустимый размер ' . ini_get('upload_max_filesize') . '');
			break;
		case '101':
			emMsg('Загрузить файлы ошибкой с кодом:' . $errorNum);
			break;
		case '102':
			emMsg('Неверный тип файла');
			break;
		case '103':
			$ret = changeFileSize(Option::UPLOADFILE_MAXSIZE);
			emMsg("Размер файла превышает максимально допустимый размер  {$ret} ");
			break;
		case '104':
			emMsg('Не удалось создать каталог загрузки файлов');
			break;
		case '105':
			emMsg('Загрузить не удалось. Каталог для вложени (content/uploadfile) не доступен');
			break;
		default:
			return $result;
			break;
...
function emMsg($msg, $url = 'javascript:history.back(-1);', $isAutoGo = false) {
	if ($msg == '404') {
		header("HTTP/1.1 404 Not Found");
		$msg = 'К сожалению, запрашиваемой Вами страницы не существует!';
	}
	echo <<<EOT
...
EOT;
	if ($url != 'none') {
		echo '<p><a href="' . $url . '">&laquo;Вернуться назад</a></p>';
	}
	echo <<<EOT
</div>
</body>
...
function emoFormat($t){
	$emos = array('[Да]'=>'0.gif', '[Улыбка]'=>'1.gif', '[Печаль]'=>'2.gif', '[Схожу с ума]'=>'3.gif', '[Плачу]'=>'4.gif', '[Люблю]'=>'5.gif', '[Ворчу]'=>'6.gif', '[Зеваю]'=>'7.gif', '[Смущаюсь]'=>'8.gif', '[Безразличен]'=>'9.gif', '[Голова кругом]'=>'10.gif', '[Подмигиваю]'=>'11.gif', '[Показваю язык]'=>'12.gif', '[Смущаюсь]'=>'13.gif', '[Удивлен]'=>'14.gif', '[Изумлен]'=>'15.gif', '[Молчу]'=>'16.gif', '[Хм]'=>'17.gif', '[Вопрос]'=>'18.gif', '[Сплю]'=>'19.gif', '[NO]'=>'20.gif', '[Плачу]'=>'21.gif', '[Целую]'=>'22.gif', '[Ржу]'=>'23.gif', '[Болею]'=>'24.gif', '[Хихикаю]'=>'25.gif', '[Думаю]'=>'26.gif', '[Цветок]'=>'27.gif', '[Сердце]'=>'28.gif', '[Разбитое сердце]'=>'29.gif', '[Кофе]'=>'30.gif', '[Музыка]'=>'31.gif', '[Дождь]'=>'32.gif', '[Солнце]'=>'33.gif', '[Звезда]'=>'34.gif', '[Луна]'=>'35.gif');
