...
class Cache {
		...
		while ($show_record = $this->db->fetch_array($query)) {
			...
			if ($record != $f_record) {
				...
				$record_cache[$p] = array(
					'record' => gmdate('Y-n', $show_record['date']),
	...
	function cacheWrite ($cacheData, $cacheName) {
	    ...
	    @ $fp = fopen($cachefile, 'wb') OR emMsg('Считывание кэша не удалось. Необходимо выставить права доступа 777 для всех файлов каталога(content/cache)');
		@ $fw = fwrite($fp, $cacheData) OR emMsg('Ошибка записи в кэш, кэш-каталог (content/cache) недоступен для записи');
