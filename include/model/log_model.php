...
class Log_Model {
	...
	function getLogsForAdmin($condition = '', $hide_state = '', $page = 1, $type = 'blog') {
		...
		while ($row = $this->db->fetch_array($res)) {
			...
			$row['title'] 	= !empty($row['title']) ? htmlspecialchars($row['title']) : 'Без названия';
			//$row['gid'] 	= $row['gid'];
	...
	function getLogsForHome($condition = '', $page = 1, $perPageNum) {
		...
		while ($row = $this->db->fetch_array($res)) {
			...
			if (!empty($row['password']) && $cookiePassword != $row['password']) {
				$row['excerpt'] = '<p>[Для доступа к статье необходимо ввести пароль]</p>';
			} else {
				if (!empty($row['excerpt'])) {
					$row['excerpt'] .= '<p class="readmore"><a href="' . Url::log($row['logid']) . '">Читать далее&gt;&gt;</a></p>';
				}
			}
	...
	function deleteLog($blogId) {
		...
		if ($this->db->affected_rows() < 1) {
			emMsg('Недостаточно прав доступа', './');
		}
		// 评论
...
Пожалуйста, введите пароль<br>
<input type="password" name="logpwd" /><input type="submit" value="Ввод" />
<br /><br /><a href="$url">&laquo;Вернуться на главную</a>
</form>
</div>
</body>
</html>
