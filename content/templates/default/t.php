...
<div id="tw">
    ...
    <div class="top"><a href="<?php echo BLOG_URL . 'admin/twitter.php' ?>">Написать в микроблог</a></div>
    <?php endif; ?>
    <ul>
    <?php 
    foreach($tws as $val):
    ...
    <div class="clear"></div>
    <div class="bttome">
        <p class="post"><a href="javascript:loadr('<?php echo DYNAMIC_BLOGURL; ?>?action=getr&tid=<?php echo $tid;?>','<?php echo $tid;?>');">Ответ: (<span id="rn_<?php echo $tid;?>"><?php echo $val['replynum'];?></span>)</a></p>
    ...
    <div class="tbutton">
        ...
        Логин: <input type="text" id="rname_<?php echo $tid; ?>" value="" />
        <span style="display:<?php if($reply_code == 'n'){echo 'none';}?>">Проверочный код: <input type="text" id="rcode_<?php echo $tid; ?>" value="" /><?php echo $rcode; ?></span>        
        </div>
        <input class="button_p" type="button" onclick="reply('<?php echo DYNAMIC_BLOGURL; ?>index.php?action=reply',<?php echo $tid;?>);" value="Ответ" /> 
